...
function loadHomePage() {
    ...
    // Обновить галерею героев
    if (heroNames && heroNames.length > 0) {
        const galleryGrid = document.querySelector('.gallery-grid');
        if (galleryGrid) {
            // Получить первые 12 карт (героев)
            const cards = getStorageCards();
            const heroCards = cards.filter(c => c.type === 'heroes').slice(0, 12);
            
            galleryGrid.innerHTML = '';
            heroCards.forEach((card, index) => {
                const heroName = heroNames[index] || card.name;
                ...
                div.innerHTML = `
                    <div class="card-preview">
                        ...
                        <div class="card-overlay">
                            <span class="card-type type-heroes">Герой</span>
                        </div>
                    </div>
...
function loadCardsPage() {
    ...
    // Если карты были изменены в админке, они уже будут обновлены в localStorage
...
// Функция для форсированного обновления контента (может быть вызвана из админки)
